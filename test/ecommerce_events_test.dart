...
void main() {
  group("Ecommerce dual-provider events", () {
    test("ViewItemEvent parameter mapping", () {
      final EcommerceItem item = const EcommerceItem(id: "b1", name: "Bundle 1", category: "esim_bundle", price: 9.99);
      final ViewItemEvent event = ViewItemEvent(item: item, platform: "android");
  ...
  expect(event.firebaseParameters["items"], isA<List<dynamic>>());
    ...
    test("AddToCartEvent computes value * quantity", () {
      final EcommerceItem item = const EcommerceItem(id: "b2", name: "Bundle 2", category: "esim_bundle", price: 5.5, quantity: 3);
      final AddToCartEvent event = AddToCartEvent(item: item, platform: "ios", currency: "eur");
    ...
    test("BeginCheckoutEvent aggregates multiple items", () {
      final List<EcommerceItem> items = <EcommerceItem>[
        const EcommerceItem(id: "b1", name: "Bundle 1", category: "esim_bundle", price: 5, quantity: 2),
        const EcommerceItem(id: "b2", name: "Bundle 2", category: "esim_bundle", price: 3),
      ];
      final BeginCheckoutEvent event = BeginCheckoutEvent(items: items, platform: "android", currency: "usd", shipping: 2, tax: 1);
      // value = 5*2 + 3*1 = 13
    ...
    test("PurchaseEvent calculates gross - discount clamped at 0", () {
      final List<EcommerceItem> items = <EcommerceItem>[
        const EcommerceItem(id: "t1", name: "Topup 1", category: "topup", price: 10),
      ];
      final PurchaseEvent event = PurchaseEvent(
        items: items,
        platform: "ios",
        currency: "usd",
        transactionId: "TX123",
